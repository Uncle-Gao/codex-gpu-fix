...
1. **Wrapper script** — replaces `Codex.app/Contents/MacOS/Codex` and launches the original binary with `--use-angle=metal` and a CDP debug port
2. **CDP persistent injector** — a long-running Node.js process that polls until the page is ready, then re-executes an idempotent payload every 2s: re-adds the `<style>` element and re-attaches a MutationObserver
3. **LaunchAgent** — watches `/Applications/Codex.app` for modifications and automatically re-applies the fix + signature after each auto-update

#### Why "persistent injection" instead of "inject once"

The initial implementation injected once and exited, but **failed at startup in practice**. Investigation showed:

1. **Codex's React mount wipes `document.head`** — a few seconds after launch our `<style>` element gets cleared along with it
2. A MutationObserver inside the payload helps, but only partially — if Codex replaces the entire `documentElement` or resets the JS context (navigation, HMR, etc.), the observer becomes orphaned
3. **Final approach: keep the injector running and re-execute the idempotent payload every 2s.** Each iteration re-attaches the observer and re-adds `<style>` if missing. Whatever Codex does to the DOM, recovery is at most 2 seconds away.
...
Electron's AsarIntegrity verifies `app.asar` with a SHA-256 hash stored under `ElectronAsarIntegrity` in `Info.plist`. Modifying the asar mismatches the hash and the app refuses to launch.

I tried recomputing the hash with `shasum -a 256` and writing it back to the plist, but Electron's hash computation differs from plain shasum (likely involving internal file ordering or metadata stripping), so the values never matched.
...
Silent by default (`>/dev/null 2>&1`).
...
I tried replacing `border-radius` with `clip-path` to get sharper corners but it didn't help. I chose to accept the current state.
...
├── scripts/
│   ├── codex-wrapper.sh               # replaces Codex.app/Contents/MacOS/Codex
│   ├── codex-inject.mjs               # CDP persistent injector (Node 22+, long-running)
│   ├── codex-fix.sh                   # auto-reapply script (invoked by LaunchAgent)
│   ├── com.uncle.codex-fix.plist      # LaunchAgent config
│   ├── cdp-toggle.mjs                 # debug panel: interactive CSS property toggles
│   └── cdp-nuclear.mjs                # debug script: test CSS properties one by one
└── notes/
    ├── 排查过程.md                     # full investigation timeline (Chinese)
    └── 失败方案.md                     # failed approaches (Chinese)
```

> ⚠️ `codex-fix.sh` and `com.uncle.codex-fix.plist` hardcode the username `uncle` and the Node path `/Users/uncle/.nvm/versions/node/v22.22.2/bin/node`. Adjust to your own machine before reuse.

### Install

> Codex is a user-mode app — `/Applications/Codex.app` is owned by the current user, so **none of these steps need sudo**.
...
# 2. Re-sign ad-hoc (do NOT use --remove-signature, macOS 13+ refuses to launch unsigned binaries)
xattr -cr /Applications/Codex.app
codesign --force --deep --sign - /Applications/Codex.app

# 3. Install the auto-reapply script (triggered after each Codex auto-update)
mkdir -p ~/.local/bin
...
# Reinstalling the official Codex app restores the original files
```
